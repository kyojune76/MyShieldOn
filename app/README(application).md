...
| 5 | 운영체제 최신 업데이트/보안 패치 정보 (OS Update/Security Patch Status) | ✅ 완료 | `Build.VERSION.RELEASE`, `SECURITY_PATCH` 등을 가져오는 로직 없음 |
| 6 | 백그라운드 과다 사용 앱 (Monitoring Background Usage) | ✅ 완료 | `UsageStatsManager` 사용 로직 없음 |
| 7 | 금융/국민앱 APK 서명 무결성 검사 (App Signature Integrity Check) | 🟡 부분 구현 | 5개 주요 앱의 SHA-256 서명 비교는 구현됨. 전체 100개 대상 DB는 준비 중 |
| 8 | ADB 모드/알 수 없는 출처 허용 상태 점검 (Developer Mode Status Check) | ✅ 완료 | `Settings.Global.ADB_ENABLED`, `Settings.Secure.INSTALL_NON_MARKET_APPS` 등 점검 로직 없음 |
